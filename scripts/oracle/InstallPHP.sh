...
for PHPVER in "${PHPVERS[@]}"; do
	...
	sudo apt-get install -y php$PHPVER-{curl,zip,intl,xmlrpc,soap,xml,gd,ldap,common,cli,mbstring,mysql,imagick,json,readline,tidy,redis,memcached,apcu,opcache,mongodb} 
done
