...
sudo add-apt-repository -y ppa:brightbox/ruby-ng # -y ??
sudo apt-get update

sudo apt-get install -y ruby2.4 ruby2.4-dev zlib1g-dev libxml2-dev libsqlite3-dev libpq-dev libxmlsec1-dev curl make g++
...
sudo bundle _1.13.6_ install --path vendor/bundle
...
CANVAS_LMS_ADMIN_EMAIL="canvas@fake.mail" # E-mail address used for default administrator login
CANVAS_LMS_ADMIN_PASSWORD=$(pwgen -s 14 1) # Password for default administrator login
CANVAS_LMS_ACCOUNT_NAME=$(pwgen -s 10 -1 -v -A -0) # Account name seen by users, usually your organization name
CANVAS_LMS_STATS_COLLECTION='anonymized' 	# opt_in, opt_out, or anonymized
