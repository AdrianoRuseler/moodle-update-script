...
# Verify if folder and data/config.ini.php exists
if [[ -d "$LOCALSITEDIR" ]]; then
	echo "$LOCALSITEDIR exists on your filesystem."
	if [ -f "$LOCALSITEDIR/data/config.ini.php" ]; then
		echo "$LOCALSITEDIR/data/config.ini.php exists!"
	else
		echo "$LOCALSITEDIR/data/config.ini.php does not exist!"
		exit 1
	fi
else
...
#DBPASS=$(cat config.ini.php | grep 'dbname' | cut -d\" -f 2) # Gets WP DB Password
