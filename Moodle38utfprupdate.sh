...
if [ -d "moodle38-plugins" ]; then
  ...
  git clone --recursive https://github.com/AdrianoRuseler/moodle38-plugins-utfpr.git
  if [[ $? -ne 0 ]]; then
    echo "Error: git clone --recursive https://github.com/AdrianoRuseler/moodle38-plugins-utfpr.git"
    echo "##------------------------ FAIL -------------------------##"
    exit 1
  fi
  cd $GIT_DIR/moodle38-plugins  
